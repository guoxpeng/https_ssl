...
- 补上「浏览器还是显示『不安全』」的排查顺序（服务端确认无误后该查什么）：
  面板本身是 HTTP 所以永远标不安全；访问的地址是否落在证书 SAN 里；CA 是否装到了
  这台设备（装完要完整退出浏览器，并给出用全新 Edge 配置直验证的命令）；
  点锁区分「证书无效」与「部分内容不安全」（后者是后端页面的混合内容）。
  见 INSTALL.md「浏览器还是显示「不安全」？」与 README 常见问题。

## [1.6.2] - 2026-09-18

### 变更（破坏性）

- **移除桥接（bridge）网络模式，host 成为唯一模式。**

  桥接模式那一套（`docker-compose.bridge.yml`、`install.sh` 的 `NETWORK` 选项、
  面板里的模式提示）全部删除。原因就是它一直在坑人：Docker 把桥接容器的端口映射
  **在创建那一刻就写死了**，运行时加不了。于是用户按面板提示填好端口、点了启用、
  容器里 nginx 也确实在监听（`docker exec` 进去 `curl` 通），从局域网访问却是
  「连接被拒」—— 很容易被当成证书或面板的 bug，而实际原因是端口从未映射到宿主机。

  host 模式下 nginx 监听的端口就是宿主机端口，面板里填了即生效，没有第二种可能。
  代价是失去网络隔离、仅 Linux 可用（Docker Desktop 请用 Linux 虚拟机），
  这些已写进 INSTALL.md / README 的对应章节。

  如果你现在的容器是 bridge 模式（用群晖 / 飞牛等图形面板建容器时容易默认选它）：
  重建容器时把网络模式选成 host 即可，**数据卷保留就不会丢证书和反代配置**。
  自检命令：`docker inspect -f '{{.HostConfig.NetworkMode}}' qilin_ssl` 应输出 `host`。

- **`QILIN_HOST_NETWORK` 环境变量作废**（既然只剩一种模式）。compose 文件、
  install.sh 与文档里对应的行已删掉；多余的变量写在环境里也不会有任何副作用。

## [1.6.1] - 2026-09-18

### 修复

- **证书主题里的中文显示成乱码**：创建 CA 时机构名填中文（比如「局域网」），
  证书装到设备后「颁发给」栏显示成双重编码的乱码字符。

  原因是 `_write_subj_config` 只写了 `string_mask = utf8only`：它只决定「输出用
  UTF8String」，而 `openssl req` 读配置里的字段值默认按 latin-1 解释，于是
  「局域网」的 9 个 UTF-8 字节被当成 9 个字符又重新编码了一遗，证书里存成了 18 字节。
  现在配置里加上 `utf8 = yes`，字段值按 UTF-8 读，主题还原为中文。

  > ⚠️ 证书的主题是签发时写死的，**已建好的 CA 及其签发的证书必须重建、重新签发**
  > 才会变成中文；链校验本身不受影响（乱码只是显示问题），旧证书仍可继续使用。

- **`_openssl()` 固定按 UTF-8 解码输出**：之前用 `text=True`，在默认编码不是 UTF-8
  的开发机（中文 Windows 是 cp936）上，只要证书主题带中文就会抛 UnicodeDecodeError，
  列表接口直接 500。

### 变更

- **CA 文件名改为 `https-ssl-ca.*`**：面板下载、文档与信任脚本统一成
  `https-ssl-ca.crt`，与项目名 https_ssl 对齐。老版本写的 `qilin-ca.key/crt/cnf/srl`
  会在启动时自动改名认领，升级复用数据卷不会丢 CA（新文件已存在时不覆盖）。
  `QILIN_*` 环境变量保持不变，无需改部署参数。
