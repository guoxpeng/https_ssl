...
2. **如何在移动设备上信任证书？**
   ...
   - iOS 还要在「设置 → 通用 → 关于本机 → 证书信任设置」里手动启用完全信任
3. **https 显示「不安全」但服务能用？**
   - 先看访问的是哪个地址：`http://<IP>:2002` 是面板本身，它就是明文 HTTP，
     永远标不安全（想绿锁就用反代给面板自己也套一层 HTTPS）
   - 再看地址在不在证书的 SAN 里（换 IP / 换域名访问就是名称不匹配），
     面板「证书验证」页可以填地址自检
   - 点地址栏的锁看具体原因：显示「部分内容不安全」是后端页面里有 `http://`
     资源（混合内容），属于后端应用的问题
   - 详细排查步骤见 INSTALL.md 的「浏览器还是显示「不安全」？」
4. **反向代理服务无法启动？**
