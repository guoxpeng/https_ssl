...
服务端正确的话，按这个顺序查基本能定位（这也是一类最常见的“明明配好了”）：

1. **看的是不是面板地址。** 面板本身是明文 HTTP（`http://<IP>:2002`），浏览器永远标
   「不安全」—— 这是正常的，`https://<IP>:2002` 更是打不开。想让它也变绿锁，
   按下一节「让面板自己也走 HTTPS」用反代套一层。
2. **访问的地址在不在证书的 SAN 里。** 证书只对签发时填的域名 / IP 有效，
   换一个 IP（比如走另一张网卡）或换个域名就是“名称不匹配”。点地址栏的锁→证书→
   看「使用者可选名称」，或直接用面板的「证书验证」页填地址自检。
3. **CA 装在这台设备上了吗。** 每台要访问的设备都得装一次（手机还有额外的
   「完全信任」开关）。装完**把浏览器完全退出再打开**：Chromium 系（Edge / Chrome）
   会把“已经点过继续访问”的证书例外**保留到整个浏览会话结束** ——
   浏览器是装 CA 之前启动的话，它会一直显示「不安全」，重启才清掉。
   5 秒自测：新开一个无痕 / InPrivate 窗口打开同一地址，它是安全的就说明
   证书已经生效，旧窗口只是残留状态（地址栏那个标记的鼠标提示也能看出来：
   「查阅站点信息」= 安全，「不安全」= 没通过）。用新配置验证一次最准：

   ```powershell
   # 能直接渲染出页面 HTML，就说明证书已被浏览器接受
   & "$env:ProgramFiles(x86)\Microsoft\Edge\Application\msedge.exe" --headless=new `
     --user-data-dir="$env:TEMP\edgecerttest" --dump-dom https://<IP>:<反代端口>/
   ```

4. **点地址栏的「不安全」看具体原因。** 显示「证书无效 / 不受信任」→ 回到第 2、3 步；
   显示「部分内容不安全」→ 后端页面里有 `http://` 的资源（混合内容），
   那是后端应用自己的写法问题，改它的资源地址或让后端也走 HTTPS 即可。
